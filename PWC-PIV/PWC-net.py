...
def predict(net, tenOne, tenTwo):
    netNetwork = net.eval().cuda()
    ...
    intPreprocessedWidth = 1024-128
    intPreprocessedHeight = 1024-128
...
if __name__ == '__main__':
    ...
    imgOne = cv2.imread('data/PIV_A.png')
    tenOne = torch.tensor(imgOne.transpose(2, 0, 1)/255).float()
    
    imgTwo = cv2.imread('data/PIV_B.png')
    tenTwo = torch.tensor(imgTwo.transpose(2, 0, 1)/255).float()

    net = Network()
    net.load_state_dict({strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.load('pwc-default').items()})
    tenOutput = predict(net, tenOne, tenTwo)
    
    u_pre = tenOutput[0][1:-1:20,1:-1:20]
    v_pre = tenOutput[1][1:-1:20,1:-1:20]
    V_pre = (u_pre**2 + v_pre**2)**0.5
    
    plt.subplots(figsize=(10, 10))
    plt.gca().invert_yaxis()
    plt.axis('off')
    
    plt.contourf(V_pre, levels=np.linspace(V_pre.min(), V_pre.max(), 30), cmap='jet')
    plt.quiver(u_pre, -v_pre)
